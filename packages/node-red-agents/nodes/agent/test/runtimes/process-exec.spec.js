...
test("timeout on a SIGTERM-ignoring process escalates to SIGKILL, killing the whole process group", async () => {
  ...
  assert.equal(
    result.signal,
    "SIGKILL",
    "the ignoring process should only die to SIGKILL, not SIGTERM",
  );
  ...
  assert.ok(
    !ps.includes('trap "" TERM'),
    "no orphan matching the killed command line should remain in the process table",
  );
});
...
test("adversarial args are passed as literal argv elements, never shell-interpreted (Direct)", async () => {
  ...
  assert.deepEqual(
    received,
    adversarial,
    "every adversarial string must arrive byte-for-byte as its own argv element",
  );
});
