...
async function waitForReady(baseUrl, timeoutMs) {
    ...
    throw new Error(`node-red did not become ready within ${timeoutMs}ms: ${lastError && lastError.message}`);
}
...
async function startSmokeInstance({ port, readyTimeoutMs = 20000 } = {}) {
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({ name: 'nra-e2e-smoke', private: true, dependencies: { '@tbrandenburg/node-red-agents': 'file:../packages/node-red-agents' } }, null, 2)
    );
    fs.writeFileSync(
