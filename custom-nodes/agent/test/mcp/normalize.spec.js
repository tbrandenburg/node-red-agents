...
test('normalizes a remote server to opencode\'s keyed mcp schema', () => {
    const out = toOpenCodeMcp([{ name: 'github', type: 'remote', url: 'https://mcp.example.com/github' }]);
...
test('normalizes a local server, folding command+args into one array', () => {
    const out = toOpenCodeMcp([
        { name: 'filesystem', type: 'local', command: 'npx', args: ['@modelcontextprotocol/server-filesystem', '/workspace'] }
    ]);
    assert.deepEqual(out, {
        filesystem: { type: 'local', command: ['npx', '@modelcontextprotocol/server-filesystem', '/workspace'], enabled: true }
    });
});
