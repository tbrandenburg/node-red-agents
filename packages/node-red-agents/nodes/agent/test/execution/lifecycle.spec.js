...
function fakeAdapter({ resultOverride } = {}) {
    return {
        ...
        parseResult(events, exitCode, signal, stderr) {
            ...
            return { payload: events.map((e) => e.data.text).join(''), status: exitCode === 0 ? 'completed' : 'failed' };
        }
    };
}
...
test('emits one onEvent per parsed line, then a single final result with status completed', async () => {
    ...
    const result = await runAgent({
        adapter: fakeAdapter(),
        runtime: fakeRuntime({ lines: [{ type: 'agent', data: { text: 'Hello' } }, { type: 'agent', data: { text: 'World' } }] }),
        resolved: {},
        executionId: 'e1',
...
test('a failed adapter result surfaces as status "failed" with an errorMessage', async () => {
    const statuses = [];
    const result = await runAgent({
        adapter: fakeAdapter({ resultOverride: { payload: '', status: 'failed', errorMessage: 'boom' } }),
...
test('runtime timedOut always wins the final status, even if the adapter thinks it completed', async () => {
    ...
    const result = await runAgent({
        ...
        runtime: fakeRuntime({ outcome: { exitCode: null, signal: 'SIGKILL', stderr: '', timedOut: true } }),
        resolved: {},
        executionId: 'e3',
        onEvent: () => {},
        onStatus: (s) => statuses.push(s)
...
test('adapter.validate() throwing prevents any execution (no onStatus("running"), rejects synchronously)', async () => {
    ...
    await assert.rejects(
        runAgent({ adapter, runtime, resolved: {}, executionId: 'e4', onEvent: () => {}, onStatus: (s) => statuses.push(s) }),
        /bad cwd/
    );
