...
test('buildExecution: skill and command invocation both map to --command <name> <args>', () => {
    ...
    assert.deepEqual(skillArgs, ['run', '--format', 'json', '--command', 'code-review', 'focus on security']);
    assert.deepEqual(commandArgs, ['run', '--format', 'json', '--command', 'review', 'focus on security']);
});
...
test('buildExecution: --session is also inserted for skill/command invocation', () => {
    ...
    const { args } = adapter.buildExecution(
        baseResolved({ invocation: 'command', invocationName: 'review', args: 'x', sessionID: 'ses_xyz' })
    );
    assert.deepEqual(args, ['run', '--format', 'json', '--session', 'ses_xyz', '--command', 'review', 'x']);
});
...
test('buildExecution: sets OPENCODE_CONFIG_CONTENT only when mcpServers is non-empty', () => {
    ...
    assert.deepEqual(parsed, { mcp: { github: { type: 'remote', url: 'https://x', enabled: true } } });
});
...
test('validate: rejects malformed mcp server entries', () => {
    const adapter = new OpenCodeAdapter();
    assert.throws(() => adapter.validate(baseResolved({ mcpServers: [{ name: 'x', type: 'remote' }] })), /requires a url/);
    assert.throws(() => adapter.validate(baseResolved({ mcpServers: [{ name: 'x', type: 'local' }] })), /requires a command/);
});
...
test('parseEvent: maps real opencode event types onto the generic vocabulary', () => {
    ...
    const text = adapter.parseEvent(JSON.stringify({ type: 'text', sessionID: 's1', part: { text: 'hi' } }));
...
test('parseResult: an {type:"error"} event fails the result even with exitCode 0', () => {
    const adapter = new OpenCodeAdapter();
    const events = [adapter.parseEvent(JSON.stringify({ type: 'error', sessionID: 's1', error: { name: 'Boom' } }))];
    const result = adapter.parseResult(events, 0, null, '');
