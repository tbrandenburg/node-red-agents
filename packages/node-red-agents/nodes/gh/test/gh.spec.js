...
function createNode(ctor, config) {
    return new ctor(Object.assign({ name: '', commandType: 'str', argsType: 'str', repoType: 'str' }, config));
}

function runInput(node, msg) {
    return new Promise((resolve) => {
        const send = (m) => { resolve({ sent: m }); };
...
test('static command/args execution: constructs correct argv and parses JSON stdout', async () => {
    const ctor = loadNode();
    const node = createNode(ctor, { command: 'pr', args: 'list --state open --json number,title,url' });
    const { sent } = await runInput(node, {});
    assert.ok(sent, 'expected a sent message, not an error');
    assert.deepEqual(sent.payload.argv, ['pr', 'list', '--state', 'open', '--json', 'number,title,url']);
